...
def evaluate_job_with_ai(job_data_dict):
    ...
    【審査規定】
    1. 基本給・月給: 最低賃金割れの懸念がないか。金額や内訳(基本給と手当の割合)が不明瞭でないか [cite: 3, 4, 8, 10]。
    2. 固定残業代: 「金額」と「時間」の両方が明記されているか [cite: 14, 15, 16]。原則45時間を超える記載(36協定の懸念)や範囲が不明確な記載がないか [cite: 21, 22, 23]。
    3. 各種手当: 手当の名称や詳細が不明なまま金額だけ記載されていないか [cite: 25, 26]。
    4. 労働時間・休日: 年間休日日数の記載が抜けていないか(必須) [cite: 30]。1日の労働時間が法定(8時間)を超えていないか [cite: 32, 33]。
    5. その他: 勤務地やタイトルなどに矛盾がないか [cite: 34, 35]。
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini", # 速度と賢さのバランスが良い最新モデル
        messages=[
            {"role": "system", "content": "あなたは求人審査の専門家です。"},
            {"role": "user", "content": prompt}
        ],
        temperature=0.0 # ルールに従わせるため、AIの創作(ブレ)をゼロにする
    )
