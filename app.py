    ...
    if st.button("✨ ミスチェック実行", type="primary"):
        if not text_a or not text_b:
            ...
            with custom_spinner('🪄 AIがくまなく探しています...'):
                ...
                - 【追加ルール③：チェック対象外項目】circus掲載内容にある「事業内容」「募集背景」「募集期間」「組織構成」の4項目については、転記ミスのチェック対象外としてください（Qmate側に記載がなくても抜け漏れエラーにはしないでください）。
                - 【追加ルール④：選考フローの固定文言】Qmate掲載内容の選考フローの箇所は、以下の指定文言と文章が合っているかチェックしてください（『』や「」などの括弧の有無や種類は除外して文章自体を確認してください）。違う場合はエラーとして指摘してください。
                  指定文言：本求人は株式会社ライフアップの採用支援サービスとなります。ご希望の際には転職活動のサポートもございます。（職業紹介事業許可番号：13-ユ-316557） 【選考の流れ】応募⇒初回面談⇒面接（1～2回程度）⇒入社
                - ただし、条件の数字の転記ミス、重要な条件の抜け漏れがあれば、「どこがどう間違っているか」を指摘してください。
                
                🛑【超重要：余計な指摘の禁止】🛑
                上記のルール（条件の抜け漏れ、数字のミス、追加ルール①〜④）に該当しないことについては、**絶対に指摘しないでください。**
                Qmateの入力ルール違反（修飾語の有無など）、明らかな誤字脱字、年齢制限などの論理的矛盾を見つけても、無視してください。勝手な粗探しは不要です。
                ...
                - 求人全体判定用NGワード: {ng_body_input} （※Qmate掲載内容のすべての文章をチェック※NGワードの「面接」に関しては「選考フロー：【選考の流れ】応募⇒初回面談⇒面接（1～2回程度）⇒入社」を除きQmate掲載内容のすべての文章をチェック）
